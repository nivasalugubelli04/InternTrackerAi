  ...
  const addSkill = async (): Promise<void> => {
    ...
    try {
      ...
      setMySkills((prev) => {
        ...
        if (exists)
          return prev.map((s) => (s.skillId === selectedSkill.id ? { ...s, proficiency } : s));
              ...
              {skills.slice(0, 20).map((skill) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.skillChip,
                      alreadyAdded && styles.skillChipAdded,
                  ...
                  >
                    <Text
                      style={[
                        ...
                        (alreadyAdded || isSelected) && styles.skillChipTextActive,
                      ]}
                    >
                      {alreadyAdded ? '✓ ' : ''}
                      {skill.name}
        ...
        <View style={styles.section}>
          ...
          {mySkills.length === 0 ? (
            <EmptyState
              emoji="⚡"
              title="No skills yet"
              description="Select a skill above and set your proficiency level."
            />
          ) : (
            mySkills.map((us) => (
              <TouchableOpacity
          ...
          {mySkills.length > 0 ? (
            <Text style={styles.hint}>Long-press a skill to remove it</Text>
          ) : null}
        </View>
      </ScrollView>
    </SafeAreaView>
